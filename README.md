...
Fetches the built area report for the **previous month** and sends it to monthly built area recipients. Always searches for the report of the month before the current month (e.g., if executed in August, sends July's report).

**Important:** The report structure in GCS must follow: `urban_sprawl/YYYY_MM/reportes/urban_sprawl_reporte_YYYY_MM.html`

**Triggering**: Call via Cloud Scheduler every Friday at 9 AM (America/Bogota). The backend now enforces "first Friday only" before sending:
```bash
...
The backend guard prevents sending outside the first Friday, and this schedule avoids daily wake-ups during the first week.
