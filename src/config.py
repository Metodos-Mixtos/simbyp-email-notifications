...
def _is_local_database_url(database_url: str) -> bool:
    """
    Detect whether DATABASE_URL points to a local PostgreSQL endpoint.

    Considered local:
    - localhost / 127.0.0.1 / ::1
    - local Unix sockets like /tmp or /var/run/postgresql
    """
    ...
    if query_host.startswith('/cloudsql/'):
        return False

    if query_host in local_hosts:
        return True

    return query_host.startswith('/tmp') or query_host.startswith('/var/run/postgresql')
